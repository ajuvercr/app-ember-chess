...
export default class CatalogService extends Service {
  ...
  async fetchRelated(record, relationship, inv_relationship = null) {
    ...
    if (isArray(json.data)) {
      ...
      if (inv_relationship) {
        const ty = record instanceof Band ? 'song' : 'band';

        for (let related of record[relationship]) {
          const related_instance = this.find(ty, (x) => x.id === related.id);
          related_instance[inv_relationship] = record;
        }
      }
    } else {
