...
export function tileUsage(game, option, isWhite) {
    if(game.enpassant)
        console.log(game.enpassant, game.enpassant.x, option.x, game.enpassant.y, option.y)
    if(game.enpassant && game.enpassant.x == option.x && game.enpassant.y == option.y) return TileState.ENPASSANT;
...
export function calculateStraightMoves(game, piece, delta)
{
    ...
    while (inRange(current) && tileUsedBy(game, current, piece.isWhite, TileState.EMPTY))
    {
    ...
    if(inRange(current) && tileUsedBy(game, current, piece.isWhite, TileState.ENEMY))
        moves.push({x: current.x, y:current.y, piece});
